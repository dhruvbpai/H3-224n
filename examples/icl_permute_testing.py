...
for j in tqdm(range(args.iters)):
    ...
    for permutation in range(args.permute_count):
        ...
        logits = normalize_probs(np.array([logits[word_maps.index(label)] for label in unique_labels]))
        ces[permutation] = cross_entropy_loss(logits[1],label_val)

        del logits
    ce_mean[j] = np.mean(ces)
    ce_stdev[j] = np.stdev(ces)

    # Free up space
    del sample

# Post processing output
np.savetxt(args.out_path+"permute_%s_%s_%s.csv"%(args.size, args.iters, args.permute_count, np.stack([ce_mean, ce_stdev]).T, delimiter=",", header=["Mean CrossEntropy, Stdev CrossEntropy"]))
